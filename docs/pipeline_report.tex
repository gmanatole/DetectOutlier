...
\begin{enumerate}[leftmargin=*]
  \item Read a profile source from NetCDF or parquet.
...
\subsection{\code{parquet.py}}
\path{src/outlierdetect/parquet.py} provides raw data export and iteration over parquet-backed profile data. This is useful when you want to stage NetCDF data once and then iterate over a lighter-weight representation later. It sits outside the core physical pipeline, but it is part of the ingest path.
...
Other workflow flags include raw-to-parquet export, prediction, training, optional heave source selection, and input toggles for residuals, uncertainty scales, and seasonal phase. Prediction still uses \code{--argo-root}, because that flag names the inference input root. Training now uses \code{--train-root}.
...
\path{src/outlierdetect/parquet.py} & Parquet staging/export & \code{write\_argo\_parquet}, \code{iter\_argo\_parquet\_profiles} \\
